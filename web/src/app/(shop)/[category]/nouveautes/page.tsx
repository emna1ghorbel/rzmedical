...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!cat) return {};
  return {
    title: `Nouveautés ${cat.nom}`,
    description: `Découvrez les nouveautés de la catégorie ${cat.nom}.`,
  };
}

export default async function CategoryNouveautesPage({ params, searchParams }: Props) {
  ...
  return (
    <CategoryCatalog
      category={category}
      searchParams={sp}
      forcedFilters={{ filter: "new" }}
      customHeading={`Nouveautés ${category.nom}`}
    />
  );
}
